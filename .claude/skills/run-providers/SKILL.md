...
```bash
node .claude/skills/run-providers/driver.mjs validate
```
...
```bash
node .claude/skills/run-providers/driver.mjs resolve <version> [channel]
```

Example:

```bash
$ node .claude/skills/run-providers/driver.mjs resolve 0.3.2 stable
GET index.json
...
1. Edit `providers/providers.YYYYMMDDHHMMSS.json` (see root `CLAUDE.md` for
   the mutable-file-update workflow).
2. `node .claude/skills/run-providers/driver.mjs validate` — if it fails on
...
- `driver.mjs resolve` only implements `^range` semver matching (the only
  syntax `index.json`'s `versions` map uses per `CLAUDE.md`) — it throws
...
- `FAIL ... duplicate model id "X"` — same `id` appears twice in one
  ...
  not been prepended alongside it (see root `CLAUDE.md`: "never delete a
