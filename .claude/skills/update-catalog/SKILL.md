---
name: update-catalog
description: Update the Routerly provider catalog — refresh model prices, add new models, deprecate old ones, add a provider key — following the workflow in root CLAUDE.md/README.md. Use for "update prices", "add a model", "deprecate model", "refresh catalog", "sync checksum", "add provider".
---
...
Per root `CLAUDE.md`/`README.md`: check the model's **official pricing page**
...
```bash
node .claude/skills/update-catalog/driver.mjs current-file
```
Prints the file the `stable` channel currently resolves to — this is the
file every command below edits (per CLAUDE.md's "mutable" update path; no
new timestamped file needed for routine changes).

**Update a price:**
```bash
node .claude/skills/update-catalog/driver.mjs set-price <provider> <modelId> <field> <value>
# field: input | output | cache | cacheWrite
node .claude/skills/update-catalog/driver.mjs set-price openai gpt-5.5 input 5.25
```
...
EOF
node .claude/skills/update-catalog/driver.mjs add-model openai /tmp/model.json
```
...
**Retire a model** (never delete — CLAUDE.md's golden rule):
```bash
node .claude/skills/update-catalog/driver.mjs deprecate openai gpt-4o
```

**Add a new provider key** (catalog side only — see note the command prints):
```bash
node .claude/skills/update-catalog/driver.mjs add-provider my-new-provider https://api.example.com/v1
```
...
hand):
```bash
node .claude/skills/update-catalog/driver.mjs sync-checksum
```
...
- The driver always edits whatever `channels[default]` resolves to (i.e.
  ...
  "only when needed" path in CLAUDE.md), do that copy/repoint by hand first,
...
- `FAIL  unknown provider key "X"` — key doesn't exist in the catalog root.
  Provider keys are never invented ad hoc; check CLAUDE.md's Provider keys
